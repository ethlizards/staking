...
contract LizardLounge is ERC721, Ownable {
    ...
    /**
     * @notice Allows user to deposit their regular Ethlizards for staking
    ...
    function depositStake(uint256[] calldata _regularTokenIds, uint256[] calldata _genesisTokenIds) external {
        ...
        /// TODO: add this back for mainnet
        ...
        for (uint256 i = 0; i < _regularTokenIds.length; i++) {
            ...
            if (!_exists(_regularTokenIds[i])) {
                ...
                emit LockedLizardMinted(msg.sender, _regularTokenIds[i]);
            } else {
                // Later deposits
    ...
    function withdrawStake(uint256[] calldata _regularTokenIds, uint256[] calldata _genesisTokenIds) external {
        ...
        for (uint256 i = 0; i < _regularTokenIds.length; i++) {
            if (originalLockedLizardOwners[_regularTokenIds[i]] != msg.sender) {
                revert CallerNotDepositer({
                    depositer: originalLockedLizardOwners[_regularTokenIds[i]],
                    caller: msg.sender
                });
            }
        ...
        for (uint256 i = 0; i < _genesisTokenIds.length; i++) {
            if (originalLockedLizardOwners[_genesisTokenIds[i]] != msg.sender) {
                revert CallerNotDepositer({
                    depositer: originalLockedLizardOwners[_genesisTokenIds[i]],
                    caller: msg.sender
                });
            }
    ...
    // TODO: ADD A MERKLE SIGNATURE HERE ONCE FRONTEND IS FINALISED
    /**
     * @notice Allows a user to claim their rewards
     * @dev When users unstake their NFT, they will lose their rewards, and the funds
     * will be locked into the contract. We also implement a signature feature to ensure
     * user are protected against securities law and are ensured participation in the DAO.
    ...
    function claimReward(uint256[] calldata _tokenIds, uint256 _poolNumber) external {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (originalLockedLizardOwners[_tokenIds[i]] != msg.sender) {
                revert CallerNotDepositer({depositer: originalLockedLizardOwners[_tokenIds[i]], caller: msg.sender});
            }

            if (isRewardsClaimed(_tokenIds[i], _poolNumber)) {
                revert RewardsClaimed({tokenId: _tokenIds[i], poolNumber: _poolNumber});
            }

            require(
                timeLizardLocked[_tokenIds[i]] < pool[_poolNumber].time,
                "Token must have been staked prior to a pool's creation"
            );
     ...
     * @param _depositAmount Amount of USDC a member wants to deposit
     */
    ...
    /**
     * @notice Checks if a lizard is transferrable
     * @dev A lizard is transferrable if it been over 90 days since it was deposited
     * @param _tokenId TokenId of the lizard
     */
    function isLizardWithdrawable(uint256 _tokenId) public view returns (bool) {
        if (block.timestamp - timeLizardLocked[_tokenId] >= 90 days) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * @notice Checks if the rewards of a lizard for a specific pool has been claimed
    ...
    /**
     * @dev Flips the state of deposits, can be called later on if we want to prevent future deposits, but won't unstake current users
     */
     ...
     * @param _tokenId TokenId where share is being calculated
     */
    ...
    /**
     * @notice Gets the current global share counter
     * @dev See technical documentation for how shares are calculated
     */
    ...
    // The address isn't the same address as the depositer
    error CallerNotDepositer(address depositer, address caller);
    ...
    error RewardsClaimed(uint256 tokenId, uint256 poolNumber);
    // Only the council can deposits rewards
